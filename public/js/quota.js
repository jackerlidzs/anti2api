...
async function loadTokenQuotaSummary(refreshToken) {
    ...
    try {
        ...
        if (error.message !== 'Unauthorized') {
            console.error('Load quota summary failed:', error);
...
async function loadQuotaDetail(cardId, refreshToken) {
    ...
    try {
        ...
        if (data.success && data.data && data.data.models) {
            ...
            if (modelEntries.length === 0) {
                detailEl.innerHTML = '<div class="quota-empty-small">No quota info</div>';
                return;
            }
            ...
            html += '</div>';
            html += `<button class="btn btn-info btn-xs quota-refresh-btn" onclick="refreshInlineQuota('${escapeJs(cardId)}', '${escapeJs(refreshToken)}')">🔄 Refresh Quota</button>`;
