...
function calculateThresholds(thresholdMB) {
  ...
  return {
    ...
    HIGH: highBytes,                        // 100% is high pressure threshold (user configured)
    TARGET: Math.floor(highBytes * 0.5)    // 50% is target memory
  };
}

// Default threshold (100MB), overridden by config at initialization
...
class MemoryManager {
   ...
   * @param {Function} callback - Cleanup function, receives pressure argument
   */
...
// Encapsulate cleanup callback registration to maintain consistent style across modules
