// Simple Memory Cache: Cache Chain of Thought signatures and tool signatures by sessionId + model
// Also integrates memory manager to automatically shrink/clear cache under high pressure
...
const CLEAN_INTERVAL_MS = 10 * 60 * 1000; // Sweep every 10 minutes
...
// Register to memory manager for automatic cache cleanup under different pressure levels
memoryManager.registerCleanup((pressure) => {
  if (pressure === MemoryPressure.MEDIUM) {
    ...
    // Critical pressure: Clear immediately, prioritize survival
...
// Reserved: Manual cleanup interface (currently unused externally, but convenient for future extension)
