...
// Buffer size warning threshold (not limited, just warning)
...
class antigravityRequester {
    ...
    _getExecutablePath() {
        ...
        // In pkg environment, prefer bin directory next to executable
        let binPath = this.binPath;
        if (!binPath) {
            if (isPkg) {
                // pkg environment: prefer bin directory next to executable
                ...
                if (fs.existsSync(exeBinDir)) {
                    ...
                    if (fs.existsSync(cwdBinDir)) {
                        ...
                        // Lastly use bin directory inside the package
    ...
    _ensureProcess() {
        ...
        this.proc.stdout.on('data', (data) => {
            ...
            // Buffer size monitoring (warning only, no limit, as image responses can be large)
            if (!this.bufferWarned && this.buffer.length > BUFFER_WARNING_SIZE) {
                console.warn(`AntigravityRequester: Buffer large (${Math.round(this.buffer.length / 1024 / 1024)}MB), possible large response`);
                this.bufferWarned = true;
            }

            this.buffer += chunk;

            // Use setImmediate to process asynchronously, avoiding blocking stdout reading
            setImmediate(() => {
                let start = 0;
                let end;

                // Efficient line splitting (avoid split creating large number of strings)
                while ((end = this.buffer.indexOf('\n', start)) !== -1) {
                    ...
                    try {
                        ...
                        // Ignore JSON parse errors (possibly incomplete lines)
                    }
                }
        ...
        this.writeQueue = this.writeQueue.then(() => {
            return new Promise((resolve, reject) => {
                ...
                if (canWrite) {
                    ...
                    // Wait for drain event, and remove other listener after any event triggers
                    const onDrain = () => {
    ...
    close() {
        if (this.proc) {
            ...
            // Send SIGTERM immediately to terminate child process, do not use setTimeout
            // This ensures child process is correctly terminated before main process exits
            try {
                if (proc && !proc.killed) {
                    proc.kill('SIGTERM');
                }
            ...
            // If SIGTERM ineffective, use SIGKILL immediately
            try {
                if (proc && !proc.killed) {
                    proc.kill('SIGKILL');
                }
