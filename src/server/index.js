...
logger.info(`Static directory: ${getRelativePath(publicDir)}`);
...
app.use((req, res, next) => {
  ...
  // Pre-fetch full path to avoid modification of req.path in route handlers
...
const shutdown = () => {
  ...
  // Close Child Process Requester
  closeRequester();
  logger.info('Child process requester closed');
  ...
  // 5 seconds timeout forced exit
  setTimeout(() => {
    logger.warn('Server shutdown timeout, forced exit');
    process.exit(0);
  }, 5000);
};
...
process.on('uncaughtException', (error) => {
  ...
  // Do not exit immediately, let current request complete
});
