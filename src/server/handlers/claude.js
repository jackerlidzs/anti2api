 ...
 * Handle /v1/messages request, support streaming and non-streaming response
 */
 ...
 * @param {boolean} isStream - Whether is streaming response
 */
export const handleClaudeRequest = async (req, res, isStream) => {
  ...
  try {
    ...
    // Use unified parameter normalization module to process Claude format parameters
    ...
    if (isStream) {
      ...
      try {
        ...
        if (isImageModel) {
          // Image generation model: Use non-streaming to get result and return in stream format
