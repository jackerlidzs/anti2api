...
function registerMemoryCleanup() {
  ...
  memoryManager.registerCleanup((pressure) => {
    // Clean model cache under HIGH or CRITICAL pressure
    ...
    if (pressure === MemoryPressure.CRITICAL && modelListCache) {
      modelListCache = null;
      modelListCacheTime = 0;
      logger.info('Emergency model list cache cleanup');
    }
  });
}
...
async function handleApiError(error, token) {
  ...
  if (status === 403) {
    ...
    throw createApiError(`Account has no permission, automatically disabled. Details: ${errorBody}`, status, errorBody);
  }
...
export async function generateAssistantResponse(requestBody, token, callback) {
  ...
  // Temporarily cache chain of thought signature in state for stream multi-fragment reuse, and carry session and model info to write to global cache
  const state = {
    toolCalls: [],
...
// Internal tool: Fetch full raw model data from remote
...
export async function getAvailableModels() {
  ...
  if (!token) {
    ...
    logger.warn('No token available, returning default model list');
  ...
  if (!data) {
    // Unified error handling done in fetchRawModels, fallback to default list here
    return getDefaultModelList();
  }
  ...
  // Add default models (if not present in API returned list)
...
export async function generateAssistantResponseNoStream(requestBody, token) {
  ...
  // Write new signature to global cache (by sessionId + model) for fallback usage in subsequent requests
...
// Export memory cleanup registration function (for external call)
