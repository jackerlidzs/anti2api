...
const RotationStrategy = {
  ROUND_ROBIN: 'round_robin',           // Round robin: Switch after every request
...
class TokenManager {
  ...
  constructor(filePath) {
    ...
    // Available token index cache for Quota Exhausted strategy (Optimize for large scale accounts)
    /** @type {number[]} */
  ...
  async _initialize() {
    try {
      ...
      if (this.tokens.length === 0) {
        log.warn('⚠ No available accounts, please add using:');
        log.warn('  Method 1: Run npm run login');
        log.warn('  Method 2: Visit web UI to add accounts');
      } else {
      ...
      log.error('Init token failed:', error.message);
      this.tokens = [];
    }
  }
  ...
  async _refreshExpiredTokensConcurrently() {
    ...
    log.info(`Concurrent refresh complete: Success ${successCount}, Fail ${failCount}, Duration ${elapsed}ms`);
  }

  /**
   * Safe refresh single token (no exception thrown)
   * @param {Object} token - Token object
   * @returns {Promise<'success'|'disable'|'skip'>} Result
   * @private
   */
  ...
  loadRotationConfig() {
    try {
      ...
      log.warn('Failed to load rotation config, using default:', error.message);
    }
  }

  // Update rotation config (Hot reload)
  ...
  // Remove specific index from quota exhausted strategy available list
  ...
  async refreshToken(token) {
    ...
    try {
      ...
      const message = typeof rawBody === 'string' ? rawBody : (rawBody?.error?.message || error.message || 'Refresh token failed');
      throw new TokenError(message, suffix, statusCode || 500);
    }
  }

  saveToFile(tokenToUpdate = null) {
    // Consistent with old interface, internally uses async write
    this.store.mergeActiveTokens(this.tokens, tokenToUpdate).catch((error) => {
      log.error('Failed to save account config:', error.message);
    });
  }

  disableToken(token) {
    log.warn(`Disable token ...${token.access_token.slice(-8)}`)
    token.enable = false;
    this.saveToFile();
  ...
  // Determine whether to switch to next token
  shouldRotate(token) {
    switch (this.rotationStrategy) {
      case RotationStrategy.ROUND_ROBIN:
        // Round Robin: Switch after every request
        return true;

      case RotationStrategy.QUOTA_EXHAUSTED:
        // Quota Exhausted: Switch check token's hasQuota flag
        // If hasQuota is false, quota exhausted, switch needed
        return token.hasQuota === false;

      case RotationStrategy.REQUEST_COUNT:
        // Switch after custom request count
  ...
  /**
   * Prepare single token (Refresh + Get projectId)
   * @param {Object} token - Token object
    ...
    if (!token.projectId) {
      if (config.skipProjectIdFetch) {
        ...
        if (projectId === undefined) {
          log.warn(`...${token.access_token.slice(-8)}: Eligible projectId not found, account disabled`);
          return 'disable';
        }
  ...
  /**
   * Token acquisition for Quota Exhausted strategy
   * @private
   */
  async _getTokenForQuotaExhaustedStrategy() {
    // If no available token currently, try to reset quota
    if (this.availableQuotaTokenIndices.length === 0) {
  ...
  /**
   * Token acquisition for default strategy (round_robin / request_count)
   * @private
   */
  ...
  async addToken(tokenData) {
    try {
      ...
      log.error('Add token failed:', error.message);
  ...
  async updateToken(refreshToken, updates) {
    try {
      ...
      log.error('Update token failed:', error.message);
      return { success: false, message: error.message };
    }
  }
  ...
  async deleteToken(refreshToken) {
    try {
      ...
      log.error('Delete token failed:', error.message);
      return { success: false, message: error.message };
    }
  }
