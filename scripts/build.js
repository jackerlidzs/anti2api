...
// Platform specific bin file map
const binFileMap = {
  ...
  'node18-macos-x64': 'antigravity_requester_linux_amd64',      // macOS x64 uses Linux version temporarily
  'node18-macos-arm64': 'antigravity_requester_android_arm64'   // macOS ARM64 uses Android version temporarily
};

console.log('📦 Step 1: Bundling with esbuild...');

// Bundle into CommonJS using esbuild
await esbuild.build({
...
function runPkg(args) {
  // Convert paths in args to forward slash format
...
try {
  ...
  // Remove old executables (avoid EPERM error)
  if (isMultiTarget) {
  ...
  if (isMultiTarget) {
    ...
    // Disable compression for ARM64 cross-compile on Windows (avoid spawn UNKNOWN error)
  ...
  if (fs.existsSync(publicSrcDir)) {
    try {
      ...
      // Copy directory using system command (more reliable)
      ...
      // Remove images directory (generated at runtime, no need to bundle)
  ...
  // Copy bin directory (only copy files for target platform)
  ...
  if (fs.existsSync(binSrcDir)) {
    ...
    // Only copy bin files for target platform
    const targetBinFiles = isMultiTarget
      ? [...new Set(targets.map(t => binFileMap[t]).filter(Boolean))]  // Multi-target: unique files
      : [binFileMap[resolvedTarget]].filter(Boolean);  // Single-target: copy one file
    ...
    if (targetBinFiles.length > 0) {
      ...
      // If no mapping, copy all files (compatibility with old behavior)
      try {
        if (process.platform === 'win32') {
  ...
  // Copy config template (copy config.json only)
