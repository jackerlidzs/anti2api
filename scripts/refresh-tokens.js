...
async function refreshAllTokens() {
  ...
  if (!tokens || tokens.length === 0) {
    log.warn('No refreshable accounts found');
    return;
  }
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    try {
      ...
      // For 400/403 errors, disable account, consistent with runtime behavior
      if (statusCode === 400 || statusCode === 403) {
        tokenManager.disableToken(token);
        log.warn(`Account ${i + 1}: Token invalid or error, account disabled automatically`);
      }
    }
  }

  log.info(`Refresh complete: Success ${successCount}, Fail ${failCount}`);
